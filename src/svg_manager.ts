...
import {
  ...
  BB_3s,
  BB_1s5,
  default_braille_dictionary,
  default_braille_moded_dictionary,
  final_BrainBraille_dictionary,
  now,
...
export class BrainBrailleStim {
  ...
  update(
    curr_l: string,
    next_l: string,
    curr_word_text: string,
    curr_l_in_word_ind: number,
    curr_index_str: string,
  ) {
...
export function genColorDefs(
  colors: string[],
  id_prefix: string,
  stop_opacity: number = 1,
  stop_offset: number = 0.2,
) {
...
export function prepBrainBrailleStim(
  ...
  const SVG_VIEW_PORT_WIDTH =
    svg_width * 2 + between_body_spacing + 2 * body_svg_to_edge_spacing;
  ...
  const next_bb_body_x_offset =
    between_body_spacing + body_svg_to_edge_spacing + svg_width;
...
export async function run_practice(
  stim_sequence: string[][],
  task_info: TaskInfo,
  brainbraille_stim: BrainBrailleStim,
) {
  ...
  const task_len = task_info.curr_l_list.length;
  let i = 0;
  let interval_id: null | ReturnType<typeof setInterval> = null;
  let my_resolve: Function;
  let my_promise = new Promise((resolve) => {
    my_resolve = resolve;
  });
  ...
  const on_key_down = (event: KeyboardEvent) => {
    ...
    if (event.key === "Escape") {
      if (interval_id) {
        set_reconnect_count(MAX_RECONNECT);
...
export function prepControlPanel(
  ...
  const ursi_label_style = {
    "font-size": "1.6em",
    "font-family": "Arial, sans-serif",
    "margin-right": "6px",
  };

  const ursi_input_form = document.createElement("form");
  const ursi_input_label = document.createElement("label");
  ursi_input_label.setAttribute("for", "ursi");
  ursi_input_label.innerHTML = "URSI:";
  ursi_input_label.setAttribute("style", obj_to_style_str(ursi_label_style));
  ursi_input_form.appendChild(ursi_input_label);
  const ursi_in = document.createElement("input");
  ursi_in.setAttribute(
    "style",
    obj_to_style_str({ ...select_style, ...{ width: "5.3em" } }),
  );
  ursi_in.setAttribute("type", "text");
  ...
  select_interval_label.setAttribute(
    "style",
    obj_to_style_str(select_label_style),
  );
  ...
  select_TR.setAttribute(
    "style",
    obj_to_style_str({ ...select_style, ...{ width: "3.9em" } }),
  );
  ...
  input_delay_s_label.setAttribute(
    "style",
    obj_to_style_str(select_label_style),
  );
