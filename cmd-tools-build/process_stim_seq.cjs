...
// src/configs_and_consts.ts
var SVG_WIDTH = 360;
var SVG_HEIGHT = 400;
var BETWEEN_BODY_SPACING = 8;
var BODY_SVG_TO_EDGE_SPACING = 6;
var DOT_RADIUS_RATIO = 1 / 10;
var DOT_COORD = [
  { x: 1 / 2, y: 1 / 11 },
  { x: 1 / 2, y: 6 / 11 },
  { x: 1 / 13, y: 3 / 11 },
...
var BB_3s = {
  ...
  expected_TR_s: 0.75,
  // This get over-written by the URL search param later
...
var final_BrainBraille_dictionary = {
  space: [0, 0, 0, 0, 0, 0],
  a: [1, 0, 0, 0, 0, 0],
  b: [0, 0, 1, 1, 0, 0],
  c: [0, 1, 0, 0, 0, 1],
...
// src/brainbraille_task_prep.ts
function generateTaskUpdateSequence(stim_phrases, task_settings = BB_3s) {
  const NUM_FRONT_SPACE = Math.ceil(
    task_settings.front_space_padding_s / task_settings.expected_task_interval_s
  );
  const NUM_BACK_SPACE = Math.ceil(
    task_settings.back_space_padding_s / task_settings.expected_task_interval_s
  );
  const curr_l_list = new Array(NUM_FRONT_SPACE).fill("space");
  const curr_word_text_list = new Array(NUM_FRONT_SPACE).fill({
    phrase_n: -1,
    word_n: -1
  });
  const curr_l_in_word_ind_list = new Array(NUM_FRONT_SPACE).fill(-1);
  for (const [phrase_i, phrase] of stim_phrases.entries()) {
    ...
    const num_extra_space = task_settings.num_space_between_sents - task_settings.num_space_between_words;
  ...
  const out_seq = {
    ...
    curr_word_text_list,
    next_l_list,
    curr_l_in_word_ind_list
  };
  return { ...out_seq, ...task_settings };
}

// src/config.ts
var DEV_WS_PORT = 8443;
var PROD_WWS_PORT = 443;
var WS_URL;
...
// src/comm.ts
var update_info_payload = new ArrayBuffer(8 + 2 * 4);
var ts_view = new Float64Array(update_info_payload, 0, 1);
var info_view = new Int16Array(update_info_payload, 8, 4);

// src/svg_manager.ts
var CURR_BODY_SVG_STYLE = {
  stroke: "#000000",
  fill: "none",
  "stroke-width": 1.5
};
var NEXT_BODY_SVG_STYLE = {
  stroke: "#b5b5b5",
  fill: "none",
  "stroke-width": 1.5
};
function obj_to_style_str(obj) {
  const str_list = [];
...
var BrainBrailleStim = class {
  ...
  curr_text_fill;
  next_text_fill;
  constructor(container_div, curr_text, next_text, curr_word, curr_index, curr_dots, next_dots, bb_dict = final_BrainBraille_dictionary, curr_text_fill = "#000000", next_text_fill = "#b5b5b5") {
  ...
  update(curr_l, next_l, curr_word_text, curr_l_in_word_ind, curr_index_str) {
    this.curr_text.innerHTML = curr_l;
    if (curr_l.length > 0) {
      this.curr_text.setAttribute("fill", this.curr_text_fill);
    }
    const curr_l_dots = this.bb_dict[curr_l];
    ...
    const next_l_dots = this.bb_dict[next_l];
...
function genColorDefs(colors, id_prefix, stop_opacity = 1, stop_offset = 0.2) {
...
function getSvgGBody(prefix = "id") {
...
function getSvgDot(id, cx, cy, r, fill) {
  return `<path id=${id} d="M ${cx},${cy} m -${r},0 a ${r},${r} 0 1,0 ${r * 2},0 a ${r},${r} 0 1,0 -${r * 2},0 z" stroke="none" fill="${fill}"/>`;
}
function prepBrainBrailleStim(parentElement, doc, svg_width = SVG_WIDTH, svg_height = SVG_HEIGHT, between_body_spacing = BETWEEN_BODY_SPACING, body_svg_to_edge_spacing = BODY_SVG_TO_EDGE_SPACING, curr_body_svg_style = CURR_BODY_SVG_STYLE, next_body_svg_style = NEXT_BODY_SVG_STYLE, dot_radius_ratio = DOT_RADIUS_RATIO, dot_coord = DOT_COORD) {
  if (typeof doc === "undefined") {
    doc = document;
  }
  const brainbraille_div = doc.createElement("div");
  ...
  const bb_svg = doc.createElementNS("http://www.w3.org/2000/svg", "svg");
  ...
  brainbraille_div.setAttribute(
    "style",
    "width: 100vw; height: 94vh; padding-top: 3vh; padding-bottom: 3vh;"
  );
  ...
  const curr_text = doc.createElementNS("http://www.w3.org/2000/svg", "text");
  curr_text.setAttribute("text-anchor", "end");
  curr_text.setAttribute(
    "x",
    `${body_svg_to_edge_spacing + svg_width * 29 / 30}`
  );
  ...
  const next_text = doc.createElementNS("http://www.w3.org/2000/svg", "text");
  next_text.setAttribute("text-anchor", "start");
  next_text.setAttribute(
    "x",
    `${body_svg_to_edge_spacing + svg_width + between_body_spacing + svg_width * 1 / 30}`
  );
  ...
  const curr_word = doc.createElementNS("http://www.w3.org/2000/svg", "text");
  curr_word.setAttribute("text-anchor", "end");
  curr_word.setAttribute(
    "x",
    `${body_svg_to_edge_spacing + svg_width * 29 / 30}`
  );
  ...
  const curr_index = doc.createElementNS("http://www.w3.org/2000/svg", "text");
  curr_index.setAttribute("text-anchor", "end");
  curr_index.setAttribute(
    "x",
    `${body_svg_to_edge_spacing + svg_width * 29 / 30}`
  );
  ...
  const curr_bb_body_g = doc.createElementNS("http://www.w3.org/2000/svg", "g");
  ...
  curr_bb_body_g.setAttribute(
    "transform",
    `translate(${body_svg_to_edge_spacing}, 0)`
  );
  bb_svg.appendChild(curr_bb_body_g);
  curr_bb_body_g.insertAdjacentHTML("beforeend", getSvgGBody());
  curr_bb_body_g.insertAdjacentHTML(
    "beforeend",
    genColorDefs(DOT_COLORS, "curr", 1, 0.32)
  );
  const curr_dot_ids = [];
  for (let i = 0; i < 6; i++) {
  ...
  const curr_dot_elements = curr_dot_ids.map((id) => doc.getElementById(id));
  const next_bb_body_g = doc.createElementNS("http://www.w3.org/2000/svg", "g");
  ...
  next_bb_body_g.setAttribute(
    "transform",
    `translate(${next_bb_body_x_offset}, 0)`
  );
  bb_svg.appendChild(next_bb_body_g);
  next_bb_body_g.insertAdjacentHTML("beforeend", getSvgGBody());
  next_bb_body_g.insertAdjacentHTML(
    "beforeend",
    genColorDefs(DOT_COLORS, "next", 0.5, 0.16)
  );
  const next_dot_ids = [];
  for (let i = 0; i < 6; i++) {
    const xy_i = dot_coord[i];
  ...
  const next_dot_elements = next_dot_ids.map((id) => doc.getElementById(id));
  return new BrainBrailleStim(
    brainbraille_div,
    curr_text,
    next_text,
    curr_word,
...
program2.command("process_text").description(
  ...
  style.innerHTML = "html{ width:1920px; height:1080px;} body {width:100%; height:100%; margin:0px; border:0; overflow: hidden; display: block;}";
  head.appendChild(style);
  const svg_parent = document2.getElementById("svg_container");
  if (svg_parent === null) {
    return;
  }
